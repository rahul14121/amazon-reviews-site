...
{
  let iTab = document.querySelector(".tabs");
  M.Tabs.init(iTab);
});
...
function signUp(userPass)
{
  ...
  const userFirstName = document.getElementById("signup-Fname").value;
  console.log(userFirstName)
  const userSurname = document.getElementById("signup-Lname").value; // Change LastName to lastName
  const userBusiness = document.getElementById("signup-Businessname").value;
  ...
  firebase.auth().onAuthStateChanged(function(user)
  {
    ...
    if (user)
    {
      ...
      const firebaseRef = firebase.database().ref('/userData/');
      const userData =
      {
